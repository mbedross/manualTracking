function [finalData]=importDataCSV_targeted(masterFilePath)
...
% coordinates of an individual particle. This is to analyze a subset of
% tracks that are positively moving in the north direction - Meant for
% analysis of the UNC data
%
...
trackPath = fullfile(masterFilePath, 'Targeted Tracks');
...
rticks([])
rticklabels({})
print(fullfile(masterFilePath,'polarHist_targeted.png'),'-dpng','-r1000')
disp('...Total Mean and STD speed is...')
absMean = mean(meanSpeed)
absSTD = std(meanSpeed)
save(fullfile(masterFilePath,'tracks_targeted.mat'), 'tracks', 'velocities', 'turningAngle', 'speed', 'meanSpeed', 'angles')
